...
/**
 * all calls are long operations, RUN IN THE BACKGROUND!!!!!!!!!!<p>
 * @author Henry
 *
 */
public class API
{
	...
	public Cube getCube(int cubeID)
	{
		...
		try
		{
			...
			if(cubeNumber!=cubeID)System.out.println("ERROR: cubeNumber!=cubeID");
			int cubeState = json.getInt("cubeState");
			String timeStamp = json.getString("timeStamp");
			String cubeStateID = json.getString("myvalue");
			return new Cube(cubeNumber,Cube.State.fromNumber(cubeState),timeStamp,cubeStateID);
		}
		catch(Exception e)
		{
			e.printStackTrace();
		...
		try
		{
			...
			for(String key:base_json.keySet())
			{
				try
				{
					...
					cubes.add(new Cube(Integer.parseInt(key),Cube.State.fromNumber(cubeState),timeStamp,Integer.toString(cubeStateID)));
				}
				catch(Exception e)
				{
					e.printStackTrace();
	...
	{
		String page = getWebsight(BASE_URL+"cube/history/"+cubeID+"/"+steps);
		JSONTokener parser = new JSONTokener(page);
		JSONObject json = (JSONObject) parser.nextValue();
		List<Cube> cubes = new ArrayList<>();
		try
		{
			for(int i = 1; i <= steps; i++)
			{
				...
				cubes.add(new Cube(cubeNumber,Cube.State.fromNumber(cubeState),timeStamp,cubeStateID));
			}
		}
		catch(Exception e)
		{
